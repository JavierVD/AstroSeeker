    ...
    try{
      ...
      if(!isConnected){
        Dialog.show({
          ...
          button: 'close',
        }); this.props.navigation.navigate('BluetoothIngles');
      }else
        console.log("XD")
    }catch(e){
      console.log("Error: " + e)
      ...
      <View style={{flex: 1}}>
        <ImageBackground
          ...
          <View style={{position: 'absolute', top: 150, left: 0}}>
            {
              (this.state.nombre=='')? <View style={{alignItems: 'center', alignContent: 'center'}}><Text style={{fontSize: 15, left: 40, fontFamily: 'astro'}}>Find a celestial object!</Text>
              <LottieView    
          ...
          <View style={{top: 140, paddingLeft: 12, paddingRight: 12}}>
            ...
            <View style={{flexDirection: 'row' ,borderRadius: 15, marginBottom:5, marginTop: 5,borderColor: "#FFF", backgroundColor: '#000',borderWidth: 2}}>
              ...
              <View>
                <NativeBaseProvider>
                <Button style={{height: 50, borderRadius: 5}} variant="subtle" onPress={()=>this.props.navigation.navigate('Calibrarin')}>
                  Align Telescope
                </Button>
                </NativeBaseProvider> 
              </View>
