    ...
    return (
      <View style={{flex: 1}}>
        ...
        {this.state.dispositivos.map((e,index)=>{
          ...
          : <TouchableOpacity onPress={()=>this.encender()}style={{borderWidth: 2, position:'absolute',width: 150,height: 30, borderColor: 'white', textAlign: 'center', left: 120,backgroundColor: 'rgba(0,0,0,0.7)'}}><Text style = {{right:-20}}>Encender bluetooth</Text></TouchableOpacity>}
        <TouchableOpacity style={{color:"#FFF", position: 'absolute',left: 160, bottom: 20,borderColor: '#FFF', borderWidth: 1, borderRadius: 90, height: 70, width: 70, alignItems: 'center', backgroundColor: '#000'}} onPress={()=> this.startDiscover()}><Text style={{top: 25}}>Refrescar</Text></TouchableOpacity>
        </ImageBackground>
      </View>
    );
  }
